...
public class Board {
	...
	private boolean killerMoveFound;
	private int rowMove;
	private int colMove;
	...
	public static final String[][] INITIALBOARD = { { "-", "-", "-", "-", "-", "-", "-", "-" },
			...
			{ "-", "-", "-", "-", "-", "-", "-", "-" } };
	// The initial board
	public static final String[][] TESTBOARD = { { "O", "X", "O", "X", "O", "X", "-", "-" },
			{ "X", "X", "O", "X", "O", "X", "-", "-" }, { "O", "X", "O", "X", "O", "X", "-", "-" },
			{ "O", "O", "X", "O", "X", "O", "-", "-" }, { "O", "X", "O", "X", "O", "-", "-", "-" },
			{ "X", "X", "O", "X", "O", "X", "-", "-" }, { "O", "X", "O", "X", "O", "X", "-", "-" },
			{ "O", "O", "X", "O", "X", "X", "-", "-" } };
	...
	public boolean isKillerMoveFound() {
		return killerMoveFound;
	}

	public void setKillerMoveFound(boolean killerMoveFound) {
		this.killerMoveFound = killerMoveFound;
	}
	...
	/**
	 * calculates whether the board is a terminal state and assigns a utility
	 * value
	 * 
	 * @return
	 */
	public boolean calcTerminalState(int i, int j, String value) {
		...
		if (j + 1 < 8 && j + 2 < 8 && j + 3 < 8) {
			leftMost = boardConfig[i][j + 1].equals(value) && boardConfig[i][j + 2].equals(value)
					&& boardConfig[i][j + 3].equals(value);
		}
		// Check if value was put at left most of three others
		if (j - 1 >= 0 && j - 2 >= 0 && j - 3 >= 0) {
			rightMost = boardConfig[i][j - 1].equals(value) && boardConfig[i][j - 2].equals(value)
					&& boardConfig[i][j - 3].equals(value);
		}
		// Check if value was put at top of three others
		if (i + 1 < 8 && i + 2 < 8 && i + 3 < 8) {
			top = boardConfig[i + 1][j].equals(value) && boardConfig[i + 2][j].equals(value)
					&& boardConfig[i + 3][j].equals(value);
		}
		// Check if value was put at the bottom of three others
		if (i - 1 >= 0 && i - 2 >= 0 && i - 3 >= 0) {
			bottom = boardConfig[i - 1][j].equals(value) && boardConfig[i - 2][j].equals(value)
					&& boardConfig[i - 3][j].equals(value);
		}
		int rowValue = 1;
